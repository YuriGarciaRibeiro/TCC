...
from config.constants import DOCKER_INPUT_DIR, DOCKER_OUTPUT_DIR, DOCKER_CONTAINER_NAME
    ...
    for root, _, files in os.walk(input_dir, topdown=False):
        ...
        for video in files[::-1]:
            if video.endswith('.mp4'):
                ...
                shutil.copy2(video_path, os.path.join(output_dir, folder_name, video))
                print(f"File {video} copied to output folder")

            elif video.endswith('.wav'):
                shutil.copy2(os.path.join(root, video), os.path.join(output_dir, folder_name, video))
                print(f"File {video} copied to output folder")
