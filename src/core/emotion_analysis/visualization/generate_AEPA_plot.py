...
from config.constants import GRAPHS_DIR
...
def generate_plot(analysis_result, sufix=""):
    ...
    plt.suptitle(
        f"Contagem das Emoções ao Longo do Vídeo: {video_name}", fontsize=16)
    ...
    table = ax3.table(cellText=cell_text, colLabels=[
                      "Chave", "Valor"], loc="center", cellLoc="center")
    ...
    actor = video_name.replace('.csv', '').split('-')[-1]
    save_dir = GRAPHS_DIR / actor
