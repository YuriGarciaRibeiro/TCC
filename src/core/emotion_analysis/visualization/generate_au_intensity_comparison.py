...
from config.constants import DATASET_DIR, GRAPHS_DIR
...
def extract_au_means(csv_path):
    ...
    au_cols = [col for col in df.columns if col.startswith(
        " AU") and col.endswith("_r")]
    means = df[au_cols].mean().to_dict()
    return {col.strip(): val for col, val in means.items()}


# Mapeia arquivos por ator e emoção
actor_files = defaultdict(lambda: defaultdict(
    lambda: {"real": None, "animated": None}))
...
for path in DATASET_DIR.rglob("*.csv"):
    ...
    emotion = next((emo for emo in EMOTIONS if emo in name), None)
    if not emotion:
        continue
...
for actor_key, emotion_dict in actor_files.items():
    for emotion in EMOTIONS:
        ...
        if not real_csv or not cg_csv:
            print(
                f"[!] Pulando {actor_key} - {emotion}: CSV real ou animado ausente.")
            continue
        ...
        ax.set_title(
            f'{actor_key.upper()} - {emotion.capitalize()}: Comparação de AUs')
        ax.set_xticks(x)
        ...
        output_dir = GRAPHS_DIR / \
            f"{actor_key}_au_intensity_comparison" / emotion
